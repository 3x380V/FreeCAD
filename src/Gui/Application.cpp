...
static bool showSplash(bool *showMessages)
{
    ...
    *showMessages = hGrp->GetBool("ShowSplasherMessages", false);

    return hGrp->GetBool("ShowSplasher", true);
}

static SplashScreen* createSplash(bool showMessages)
{
    SplashScreen* splash = new SplashScreen(Gui::splashImage(),
        Qt::WindowStaysOnTopHint | Qt::X11BypassWindowManagerHint);

    splash->setShowMessages(showMessages);
    splash->show();
...
void Application::runApplication()
{
    ...
    SplashScreen *splash = nullptr;
    bool showMessages;
    if (Gui::showSplash(&showMessages)) {
        splash = Gui::createSplash(showMessages);
    }
...
void Application::destroySplash()
{
    if (d->splash) {
        d->splash->close();
        delete d->splash;
