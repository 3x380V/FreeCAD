...
/// Identity of the bytes a StringIDRef points at, for shared/copied storage checks.
const void* rawData(const App::StringIDRef& id)
{
    return static_cast<const void*>(id.deref().data().data());
}
...
TEST_F(StringIDRefTest, constData)  // NOLINT
{
    ...
    // Act
    const auto& data = sid.deref().data();

    // Assert
    ASSERT_NE(data.data(), nullptr);
    EXPECT_EQ(toStdString(data), "data");
}
...
TEST_F(StringHasherTest, getIDFromQByteArrayShort)  // NOLINT
{
    ...
    // Assert
    EXPECT_EQ(toQByteArray(id.deref().data()), qba);
    EXPECT_FALSE(id.isHashed());
    // A copy was made, the pointers differ
    EXPECT_NE(static_cast<const void*>(qba.constData()), rawData(id));
...
TEST_F(StringHasherTest, getIDFromQByteArrayLongHashable)  // NOLINT
{
    ...
    // Assert
    EXPECT_NE(toQByteArray(id.deref().data()), qba);
...
TEST_F(StringHasherTest, getIDFromQByteArrayNoCopy)  // NOLINT
{
    ...
    // Assert
    EXPECT_EQ(toQByteArray(id.deref().data()), qba);
    // No copy was made, the pointers are the same
    EXPECT_EQ(static_cast<const void*>(qba.constData()), rawData(id));
}
