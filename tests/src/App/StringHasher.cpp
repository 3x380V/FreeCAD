...
TEST_F(StringIDRefTest, toBytesPreservesEmbeddedNul)  // NOLINT
{
    // Arrange
    const std::string data {"da\0ta", 5};
    const std::string postfix {"post"};
    const std::string expected {"da\0ta2post", 10};
    auto* stringID = new App::StringID {
        1,
        Base::ByteBuffer::copy(Base::asBytes(data.data(), data.size()))
    };
    stringID->setPostfix(Base::ByteBuffer::copy(Base::asBytes(postfix)));
    auto sid = App::StringIDRef(stringID, 2);

    // Act
    const Base::ByteBuffer bytes = sid.toBytes();

    // Assert
    EXPECT_TRUE(Base::equalBytes(bytes.view(), Base::asBytes(expected.data(), expected.size())));
}
...
TEST_F(StringIDRefTest, toBytes)  // NOLINT
{
    ...
    // Act
    const Base::ByteBuffer byteStorage = ref.toBytes();
...
TEST_F(StringHasherTest, getIDFromQByteArrayShort)  // NOLINT
{
    ...
    // Assert
    const Base::ByteBuffer idBytes = id.toBytes();
    EXPECT_TRUE(Base::equalBytes(idBytes.view(), Base::asBytes(string.data(), string.size())));
    EXPECT_FALSE(id.isHashed());
    EXPECT_NE(Base::asBytes(qba.constData(), qba.size()).data(), id.deref().data().data());
...
TEST_F(StringHasherTest, getIDFromQByteArrayLongHashable)  // NOLINT
{
    ...
    // Assert
    const Base::ByteBuffer idBytes = id.toBytes();
    EXPECT_FALSE(Base::equalBytes(idBytes.view(), Base::asBytes(string.data(), string.size())));
...
TEST_F(StringHasherTest, getIDFromQByteArrayNoCopy)  // NOLINT
{
    ...
    const Base::ByteBuffer idBytes = id.toBytes();
    EXPECT_TRUE(Base::equalBytes(idBytes.view(), Base::asBytes(string.data(), string.size())));
    EXPECT_EQ(Base::asBytes(qba.constData(), qba.size()).data(), id.deref().data().data());
}
